...
async function ban(client, message) {
  ...
  await utils.queryDB('INSERT INTO cases (guild_id, type) VALUES (?, ?)', [message.guild.id, 'ban'])
  return message.channel.send(mentionedMember.user.tag + ' has been banned.')
}

const command = new Command(ban, 'ban', 'Ban', 'Moderation', 'ban [@member] [days worth of messages] [reason]', ['ban'], 'Bans the mentioned member, removing the specified amount of days worth of messages (or 7 by default).', Permissions.FLAGS.BAN_MEMBERS, true)
